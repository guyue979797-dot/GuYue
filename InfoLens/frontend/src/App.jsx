import "./styles.css";
import { TableRowActions } from "./components/TableActions.jsx";
...
function RowActions({ onDetail, items }) {
  return (
    <TableRowActions
      Dropdown={Dropdown}
      Menu={Menu}
...
function ProgressBar({ percent, completed = false, overdue = false, label }) {
  const normalized = Math.max(0, Math.min(100, Number(percent) || 0));
  const stateClass = overdue ? "overdue" : completed ? "completed" : "";
  return (
    <div className="progress-track progress-bar-track" aria-label={label}>
      <div
        className={`progress-bar progress-bar-fill ${stateClass}`.trim()}
...
export function TableEllipsis({
  ...
  return (
    <Ellipsis
      ...
      rows={1}
      showTooltip={tooltipContent === "-" ? false : {
        content: tooltipContent,
        getPopupContainer: () => document.body,
      }}
      aria-label={tooltipContent === "-" ? undefined : tooltipContent}
        ...
        <div className="operation-toolbar">
          ...
          <div className="selection-summary">
            ...
            <div className="selection-metric">
              ...
              <span className="tag-neutral numeric-tag">{selected.size}</span>
...
function CustomerManagement({ csrfToken, isAdmin, currentUser }) {
  ...
  const customerTableWrapRef = useRef(null);
  const customerFrozenLeftRef = useRef(null);
  const customerFrozenRightRef = useRef(null);
  ...
  function syncCustomerFrozenLayers() {
    const container = customerTableWrapRef.current;
    if (!container) return;
    const scrollLeft = container.scrollLeft;
    customerFrozenLeftRef.current?.style.setProperty("transform", `translate3d(${scrollLeft}px, 0, 0)`);
    customerFrozenRightRef.current?.style.setProperty(
      "transform",
      `translate3d(${scrollLeft + container.clientWidth - 150}px, 0, 0)`,
    );
  }

  useEffect(() => {
    syncCustomerFrozenLayers();
    window.addEventListener("resize", syncCustomerFrozenLayers);
    return () => window.removeEventListener("resize", syncCustomerFrozenLayers);
  }, [items.length]);
  ...
  async function loadCustomers() {
    ...
    (appliedFilters.route || []).forEach((route) => params.append("route", route));
    (appliedFilters.people || []).forEach((person) => params.append("person", person));
    try {
    ...
    <div className="customer-page">
      <Card bordered className="customer-filter-card">
        <div className="customer-filter-grid">
          ...
          >
            {[...new Set([...CUSTOMER_OPTIONS.salespeople, ...CUSTOMER_OPTIONS.snowSalespeople])]
      ...
      <Card bordered className="customer-list-card">
        ...
        <div className="customer-table-shell">
          <div
            ...
            onScroll={syncCustomerFrozenLayers}
          >
            <table className="customer-table">
              <colgroup>
                <col className="customer-col-code" />
                <col className="customer-col-name" />
                <col className="customer-col-route" />
                <col className="customer-col-salesperson" />
                <col className="customer-col-snow-salesperson" />
                <col className="customer-col-status" />
                <col className="customer-col-policy" />
                <col className="customer-col-remark" />
              </colgroup>
              <thead>
                <tr>
              ...
              <tbody>
                {items.map((customer) => (
                  <tr
                    key={customer.id}
                    className={customer.status === "运营" ? "" : "disabled"}
            ...
            <div
              className="customer-frozen-left"
              ref={customerFrozenLeftRef}
              aria-hidden="true"
            >
              <div className="customer-frozen-left-head">
                <span>终端编码</span>
                <span>客户全名</span>
              </div>
              {items.map((customer) => (
                <div
                  className={`customer-frozen-left-row ${customer.status === "运营" ? "" : "disabled"} ${hoveredCustomerId === customer.id ? "is-hovered" : ""}`.trim()}
                  key={`frozen-${customer.id}`}
                  onMouseEnter={() => setHoveredCustomerId(customer.id)}
                  onMouseLeave={() => setHoveredCustomerId(null)}
                >
                  <span className="terminal-code">{customer.terminal_code}</span>
                  <span className="customer-frozen-name">{customer.customer_name || "-"}</span>
                </div>
              ))}
            </div>
            <div className="customer-frozen-right" ref={customerFrozenRightRef}>
              <div className="customer-frozen-right-head">操作</div>
              {items.map((customer) => (
                <div
                  className={`customer-frozen-right-row ${customer.status === "运营" ? "" : "disabled"} ${hoveredCustomerId === customer.id ? "is-hovered" : ""}`.trim()}
                  key={`actions-${customer.id}`}
                  onMouseEnter={() => setHoveredCustomerId(customer.id)}
                  onMouseLeave={() => setHoveredCustomerId(null)}
                >
                  <RowActions
                    onDetail={() => openCustomerDetail(customer)}
                    items={[
                      { key: "edit", label: "编辑", onClick: () => openEdit(customer) },
                      { key: "logs", label: "修改记录", onClick: () => openLogs(customer) },
                      ...(isAdmin ? [{ key: "delete", label: "删除", danger: true, onClick: () => deleteCustomer(customer) }] : []),
                    ]}
                  />
                </div>
              ))}
                ...
                return (
                  <div className="snow-rule-card" key={`${rule.tag}-${ruleIndex}`}>
                    <div className="snow-rule-head">
                      ...
                      <span className="tag-neutral">{rule.tag}</span>
                      <Button
                        size="mini"
                        status="danger"
      ...
      <Card bordered className="product-list-card">
        ...
        <div className="product-table-wrap">
          <table className="product-table">
            ...
            <tbody>
              {items.map((product) => (
                <tr key={product.id} className={product.status === "正常" ? "" : "disabled"}>
                  <td>
                    {product.short_name ? (
                      <span
                        className="product-short-tag tag-neutral"
                      >
                        {product.short_name}
                      </span>
                  ...
                  <td className="product-actions-sticky">
                    <RowActions
                      ...
                      items={[
                        { key: "edit", label: "编辑", onClick: () => openEdit(product) },
                        { key: "delete", label: "删除", danger: true, onClick: () => deleteProduct(product) },
                      ]}
                    />
                  </td>
                </tr>
...
function SnowPolicyManagement({ csrfToken, isAdmin }) {
  ...
  function togglePolicySort(field) {
    setPage(1);
    setSortConfig((current) => ({
      field,
      order:
        current.field === field && current.order === "desc"
          ? "asc"
          : "desc",
    }));
  }
          ...
          <table className="policy-table">
            <thead>
              <tr>
                ...
                <th>出库编码</th>
                <th>月目标</th>
                <th className="outbound-progress-header">
                  <button
                    className={sortConfig.field === "shipped_count" ? "policy-sort-button active" : "policy-sort-button"}
                    type="button"
                    title="点击切换已出库排序"
                    onClick={() => togglePolicySort("shipped_count")}
                  >
                    <span>出库进度</span>
                    <span className="policy-sort-arrows" aria-hidden="true">
                      <i className={sortConfig.field === "shipped_count" && sortConfig.order === "asc" ? "selected" : ""}>▲</i>
                      <i className={sortConfig.field === "shipped_count" && sortConfig.order === "desc" ? "selected" : ""}>▼</i>
                    </span>
                  </button>
                </th>
                <th>套数限制</th>
                ...
                <th>
                  <button
                    className={sortConfig.field === "reimbursement_amount" ? "policy-sort-button active" : "policy-sort-button"}
                    type="button"
                    title="点击切换核销金额排序"
                    onClick={() => togglePolicySort("reimbursement_amount")}
                  >
                    <span>核销金额</span>
                    <span className="policy-sort-arrows" aria-hidden="true">
                      <i className={sortConfig.field === "reimbursement_amount" && sortConfig.order === "asc" ? "selected" : ""}>▲</i>
                      <i className={sortConfig.field === "reimbursement_amount" && sortConfig.order === "desc" ? "selected" : ""}>▼</i>
                    </span>
                  </button>
                </th>
            ...
            <tbody>
              {items.map((policy) => {
                const shipped = Number(policy.shipped_count || 0);
                return (
                <tr key={policy.id} className={policy.enabled ? "" : "disabled"}>
                  ...
                  <td><code className="policy-outbound-code">{policy.outbound_code || "-"}</code></td>
                  <td>{policy.month_target ?? "-"}</td>
                  <td className="outbound-progress-cell">
                      <PolicyProgressCell
                        ...
                        onShippedClick={() => openPolicyTerminals(policy, "shipped")}
                        onPhotographedClick={() => openPolicyTerminals(policy, "photographed")}
                        onPendingClick={() => openPolicyTerminals(policy, "pending")}
                        onAlertClick={() => openPolicyAlerts(policy)}
                        onReversedClick={() => openPolicyTerminals(policy, "reversed")}
                      />
                  </td>
                  <td><span className="policy-metric-number">{policy.set_limit ?? "-"}</span></td>
                  <td className="policy-photo-check-cell">
                    <span
                  ...
                  <td><span className="policy-metric-number">{formatPolicyQuantity(policy.reimbursement_quantity)}</span></td>
                  <td>
                  ...
                  <td className="policy-actions-sticky">
                    <RowActions
                      onDetail={() => openPolicyDetail(policy)}
                      items={[
                        { key: "edit", label: "编辑", onClick: () => openPolicyEdit(policy) },
                        { key: "export", label: exportingPolicyId === policy.id ? "导出中" : "导出", disabled: Boolean(exportingPolicyId), onClick: () => exportPolicy(policy) },
                        ...(isAdmin ? [{ key: "delete", label: "删除", danger: true, onClick: () => deletePolicy(policy) }] : []),
                      ]}
                    />
                  </td>
                </tr>
          ...
          <Header className="app-header">
            <div className="topbar">
              <nav className="breadcrumbs" aria-label="当前位置">
                <button type="button" className="breadcrumb-link" onClick={() => setActivePage("library")}>首页</button>
                <span className="breadcrumb-separator" aria-hidden="true">›</span>
                <button
                  type="button"
                  className="breadcrumb-link"
                  onClick={() => setActivePage(activePage === "customers" ? "customers" : activePage)}
                >
